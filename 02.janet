...
(def peg
  ~{:reading (group (* (number :d+) " " :color))
    :round (/ (group (some (* :reading (? ", ")))) ,round-to-table)
    :color (+ (/ "blue" :blue) (/ "red" :red) (/ "green" :green))
    :rounds (some (* :round (? "; ")))
    :game (/ (* "Game " (number :d+) ": " (group :rounds) (? "\n")) ,|{:id $0 :rounds $1})
    :main (some :game)})
...
(test (round-max [{:blue 3 :red 4} {:blue 6 :green 2 :red 1} {:green 2}]) @{:blue 6 :green 2 :red 4})
...
(test (zip-all <=* {:a 1} {:a 4}) true)
(test (zip-all <=* {:a 4} {:a 2}) false)
(test (zip-all <=* {} {:a 1}) true)
(test (zip-all <=* {:red 10} {:green 20}) true)

(defn solve [input]
  (def possible-games (->> (peg/match peg input)
    (filter (fn [{:id id :rounds rounds}]
      (zip-all <=* (round-max rounds) {:red 12 :green 13 :blue 14})))))
...
(defn solve2 [input]
  (sum (seq [{:id id :rounds rounds} :in (peg/match peg input)]
    (product (values (round-max rounds))))))
